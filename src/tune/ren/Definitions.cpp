...
Variable validationSplit("validationSplit", "Fraction of the training data to use for validation", 0.1);
Variable k("k", "Factor multiplied with the evaluation value inside the tanh function", 0.0025);
Variable learningRate("learningRate", "Learning rate for the optimizer at generation 0", 1e-3);
Variable learningRateDecay("learningRateDecay", "Learning rate decay factor with each generation", 1.0);
Variable numEpochs("numEpochs", "Number of epochs for training at generation 0", 8000ull);
...
Variable noImprovementPatience("noImprovementPatience", "Number of generations without improvement before stopping training", 4ull);
Variable batchSize("batchSize", "Batch size for training", 512ull);
