...
Variable temperature("temperature", "Temperature parameter in centipawns for move selection during simulation (0 = greedy, inf = uniform random)", 50.0);
...
Variable learningRate("learningRate", "Learning rate for the optimizer at generation 0", 3e-4);
...
Variable batchSize("batchSize", "Batch size for training", 1024ull);
...
Variable lambda("lambda", "Base lambda parameter for TD(lambda) updates", 1.0);
Variable useDynamicLambda("useDynamicLambda", "Whether to use dynamic lambda", true);
...
Variable eloAddPeriod("eloAddPeriod", "Add a new generation snapshot to Elo table every N generations", 20ull);
Variable eloUpdatePeriod("eloUpdatePeriod", "Update Elo table every N generations", 5ull);
Variable eloNumGamesPerUpdate("eloNumGamesPerUpdate", "Number of games to play when updating the Elo table", 100ull);
