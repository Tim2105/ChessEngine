...
Variable numGames("numGames", "Number of games to simulate at generation 0", 12ull);
Variable numGamesIncrement("numGamesIncr", "Number of games to simulate more with each generation", 0ull);
Variable timeControl("timeControl", "Time control in ms at generation 0", 160000ull);
Variable increment("increment", "Time control increment in ms at generation 0", 16000ull);
Variable timeGrowth("timeGrowth", "Time control growth factor with each generation", 1.0);
Variable openingBookMovesMin("openingBookMovesMin", "Minimum number of half moves to play from opening positions before simulation", 8ull);
...
Variable noiseDefaultStdDev("noiseDefaultStdDev", "Default standard deviation for noise", 0.4);
Variable noiseLinearStdDev("noiseLinearStdDev", "Additional standard deviation for noise based on the absolute value of the parameter", 0.02);
Variable noiseDecay("noiseDecay", "Noise decay factor with each generation", 0.97);
...
Variable learningRate("learningRate", "Learning rate for the optimizer at generation 0", 0.1);
Variable learningRateDecay("learningRateDecay", "Learning rate decay factor with each generation", 0.99);
Variable numEpochs("numEpochs", "Number of epochs for training at generation 0", 3ull);
